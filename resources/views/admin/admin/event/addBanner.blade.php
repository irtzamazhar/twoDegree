        ...
        <section class="invoice">
            <div>
                <div class="box-body">
                    ...
                    @foreach($siteBanner as $eventBanner)
                        <div class="form-group">
                            <img src="{{ asset('storage/app/public/images/'. $eventBanner['banner_image']) }}" width="100%">
                        </div>
                    @endforeach
                    <form action="{{ action('SiteEventsController@storeBanner', $eventBanner['id']) }}" method="post" id="blogpost" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="form-group {{ $errors->has('banner-image') ? ' has-error' : '' }}">
                            <label for="banner-image">Banner Image</label>
                            <input type="hidden" id="page-name" name="page-name" value="event">
